...
export class PayrollComponent implements OnInit {
  ...
  payrollToCreate = new SPCSObject;
  payrollToUpdate = new SPCSObject;
        ...
        res => {
          ...
          for (let i = 0; i < this.payrolls.length; i++) {
            ...
            switch (el.Status) {
              case 1:
                el.Status2 = "Mới";
                el.BtnImport = `<i id = "Import_` + el.Id + `" style="cursor: pointer;" class="fa fa-share-square-o fa-2x ast-black trigger-modal-import" ></i>`;
                el.BtnEdit = `<i id = "Edit_` + el.Id + `" style="cursor: pointer;" class="fa fa-cloud-upload fa-2x ast-yellow"></i>`;
                el.BtnPrint = `<i id = "Print_` + el.Id + `" style="cursor: not-allowed" class="fa fa-cloud-download fa-2x fa-disabled ast-black"></i>`;
                break;
              case 2:
                el.Status2 = "Đã đăng tải";
                el.BtnImport = `<i id = "Import_` + el.Id + `" style="cursor: pointer;" class="fa fa-share-square-o fa-2x ast-black trigger-modal-import" ></i>`;
                el.BtnEdit = `<i id = "Edit_` + el.Id + `" style="cursor: pointer;" class="fa fa-cloud-upload fa-2x ast-yellow"></i>`;
                el.BtnPrint = `<i id = "Print_` + el.Id + `" style="cursor: pointer" class="fa fa-cloud-download fa-2x ast-black"></i>`;
                break;
              case 3:
                el.Status2 = "Đã thanh toán";
                el.BtnImport = `<i id = "Import_` + el.Id + `" style="cursor: not-allowed;" class="fa fa-share-square-o fa-2x fa-disabled ast-black trigger-modal-import" ></i>`;
                el.BtnEdit = `<i id = "Edit_` + el.Id + `" style="cursor: not-allowed;" class="fa fa-cloud-upload fa-2x fa-disabled ast-yellow"></i>`;
                el.BtnPrint = `<i id = "Print_` + el.Id + `" style="cursor: pointer" class="fa fa-cloud-download fa-2x ast-black"></i>`;
                break;
              case 4:
                el.Status2 = "Quá hạn thanh toán";
                el.BtnImport = `<i id = "Import_` + el.Id + `" style="cursor: not-allowed;" class="fa fa-share-square-o fa-2x fa-disabled ast-black trigger-modal-import" ></i>`;
                el.BtnEdit = `<i id = "Edit_` + el.Id + `" style="cursor: not-allowed;" class="fa fa-cloud-upload fa-2x fa-disabled ast-yellow"></i>`;
                el.BtnPrint = `<i id = "Print_` + el.Id + `" style="cursor: pointer" class="fa fa-cloud-download fa-2x ast-black"></i>`;
                break;
  ...
  getActiveDocument() {
    ...
    this.api.getActiveDocument(this.payrollToCreate.Month, this.payrollToCreate.Year).subscribe(
      res => {
        console.log(res);
  ...
  publishPayroll() {
    this.api.publishPayroll(this.payrollToUpdate.Id)
      .subscribe(
        res => {
          Swal.fire("Thành Công", "Đăng tải thành công", "success");
          this.ngOnInit();
        },
        err => {
  ...
  downloadPayroll() {
    ...
    location.href = this.api.getApiUrl() + "Payroll/DownloadPayroll?payrollId=" + this.payrollToUpdate.Id;

    // this.api.downloadPayroll(this.payrollToUpdate.Id).subscribe(
    //   res => {
    //     location.href = this.api.getApiUrl + "Payroll/DownloadPayroll?payrollId=" + this.payrollToUpdate.Id;
    //   },
    //   err => {
    //     Swal.fire("Cảnh Báo", "Bảng lương chưa được đăng tải", "warning");
    //   }
    // )
  ...
  openMonthlyModal(payrollId) {
    ...
    this.modalRef = this.modalService.show(
      ModalMonthlyComponent, { initialState }
    );
  ...
  openDownloadPayrollModal() {
    const initialState = {
      vm: this
    };

    this.modalRef = this.modalService.show(
      ModalDownloadPayrollComponent,
      {
        initialState: initialState,
        // ignoreBackdropClick: true
      }
...
export class ModalMonthlyComponent implements OnInit {
  ...
  excelFileSalary: any;
  activeSalary = false;
  btnStateSalary = "Mở thêm thông tin lương nhân viên";
  excelFileMonthly: any;
  ...
  complete() {
    if (this.excelFileMonthly == null && this.excelFileSalary == null) {
      Swal.fire("Cảnh Báo", "Chưa upload file thông tin lương", "warning");
    }
    else {
      // this.UploadSalary();
      this.UploadMonthly();
  ...
  onFileChange(evt: any) {
    ...
    reader.onload = (e: any) => {
      ...
      /* save data */
      this.excelFileMonthly = <XLSX.AOA2SheetOpts>(XLSX.utils.sheet_to_json(ws, { header: 1 }));
  ...
  // downloadMonthly() {
  //   this.api.downloadMonthlyFull([], []).subscribe(
  //     res => {
  //       var blob = new Blob([res], {
  //         type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
  //       });
  //       saveAs(blob, 'Mẫu Thông Tin Lương Hàng Tháng' + '.xlsx');
  //     },
  //     err => {
...
@Component({
  selector: 'modal-publish',
  template: `
  <div class="modal-header">
    <h5 style="margin-left: 115px; font-size: 20px" class="modal-title">Công Khai Bảng Lương</h5>
  </div>
  <div class="modal-footer">
    <button type="button" class="btn btn-danger" (click)="fieldModalRef.hide()">
        <i class="fa fa-times"></i>
        Hủy</button>
    <button style="margin-right: 135px;" type="button" class="btn btn-success" (click)="this.vm.publishPayroll()">
        <i class="fa fa-check-square-o"></i>
        Xác nhận</button>
  </div>
    `
...
@Component({
  ...
  template: `
  <div class="modal-header">
    <h5 style="margin-left: 115px; font-size: 20px" class="modal-title">Tải Xuống Bảng Lương</h5>
  </div>
  <div class="modal-footer">
    <button type="button" class="btn btn-danger" (click)="fieldModalRef.hide()">
        <i class="fa fa-times"></i>
        Hủy</button>
    <button style="margin-right: 135px;" type="button" class="btn btn-success" (click)="this.vm.downloadPayroll()">
        <i class="fa fa-check-square-o"></i>
        Xác nhận</button>
  </div>
    `
...
export class ModalDownloadPayrollComponent implements OnInit {
